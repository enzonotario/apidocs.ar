...
async function fetchLocalidades(provincia, municipio, localidadCensal) {
  const url = `https://apis.datos.gob.ar/georef/api/localidades?provincia=${encodeURIComponent(provincia)}&municipio=${encodeURIComponent(municipio)}&localidad_censal=${encodeURIComponent(localidadCensal)}&orden=id&aplanar=true&campos=completo&max=10&inicio=0&exacto=true&formato=json`
  const data = await (await fetch(url)).json()
  localidades.value = data.localidades || []
}
...
const chartOptionsLocalidades = ref({
  responsive: true,
  plugins: {
...
watch(selectedLocalidadCensal, async (loc) => {
  if (selectedProvincia.value && selectedMunicipio.value && loc) {
    await fetchLocalidades(selectedProvincia.value, selectedMunicipio.value, loc)
    await nextTick()
    const bounds = getBounds(localidades.value)
...
<template>
  <div class="relative">
    ...
    <h3 class="mt-6">Distribución geográfica de Localidades de {{ selectedLocalidadCensal }}, {{ selectedMunicipio }}, {{ selectedProvincia }}</h3>
    ...
    <h3>Localidades de {{ selectedLocalidadCensal }}, {{ selectedMunicipio }}, {{ selectedProvincia }}</h3>
    <table v-if="localidades.length">
      <thead>
      <tr>
        <th>Nombre</th>
        <th>Categoría</th>
      ...
      <tbody>
      <tr v-for="l in localidades" :key="l.id">
        <td>{{ l.nombre }}</td>
        <td>{{ l.categoria }}</td>
        <td>{{ l.fuente }}</td>
        <td>{{ l.centroide_lat?.toFixed(4) }}</td>
        <td>{{ l.centroide_lon?.toFixed(4) }}</td>
        <td>{{ l.departamento_nombre }}</td>
        <td>{{ l.municipio_nombre }}</td>
        <td>{{ l.provincia_nombre }}</td>
        <td>{{ l.localidad_censal_nombre }}</td>
      </tr>
      </tbody>
    </table>
  </div>
