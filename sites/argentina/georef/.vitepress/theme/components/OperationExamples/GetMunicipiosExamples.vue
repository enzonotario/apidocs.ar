...
async function fetchMunicipios(provinciaNombre) {
  const url = `https://apis.datos.gob.ar/georef/api/municipios?provincia=${encodeURIComponent(provinciaNombre)}&orden=id&aplanar=true&campos=completo&max=1000&inicio=0&exacto=true&formato=json`
  const data = await (await fetch(url)).json()
  municipios.value = data.municipios
}

function getBounds(muns) {
  if (!muns.length) return { minLat: -34, maxLat: -30, minLon: -63, maxLon: -60 }
  let minLat = Infinity, maxLat = -Infinity, minLon = Infinity, maxLon = -Infinity
  for (const m of muns) {
    if (m.centroide_lat < minLat) minLat = m.centroide_lat
    if (m.centroide_lat > maxLat) maxLat = m.centroide_lat
    if (m.centroide_lon < minLon) minLon = m.centroide_lon
    if (m.centroide_lon > maxLon) maxLon = m.centroide_lon
  }
...
watch(selectedProvincia, async (prov) => {
  if (prov) {
    ...
    await nextTick()
    const bounds = getBounds(municipios.value)
...
<template>
  <div class="relative">
    ...
    <table>
      ...
      <tbody>
      <tr v-for="mun in municipios" :key="mun.id">
        <td>{{ mun.nombre_completo }}</td>
        <td>{{ mun.nombre }}</td>
        <td>{{ mun.categoria }}</td>
        <td>{{ mun.fuente }}</td>
        <td>{{ mun.centroide_lat.toFixed(4) }}</td>
        <td>{{ mun.centroide_lon.toFixed(4) }}</td>
        <td>{{ mun.provincia_interseccion.toFixed(4) }}</td>
      </tr>
      </tbody>
    </table>
  </div>
